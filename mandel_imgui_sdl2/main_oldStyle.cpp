...
static const char *appTitle {"wgpu - Mandelbrot - SDL example"};
...
wgpu::Queue                 queue;
wgpu::TextureFormat         preferredFormat { wgpu::TextureFormat::Undefined };  // current undefined, but set/acquired from SurfaceCapabilities
...
void checkMouseButtonAction()
{
    ...
    if(!mouseCaptured && SDL_GetMouseState(&x, &y) == SDL_BUTTON(SDL_BUTTON_LEFT))
        zoom(-zoomFactor);
    else if(!mouseCaptured && SDL_GetMouseState(&x, &y) == SDL_BUTTON(SDL_BUTTON_RIGHT))
        zoom(zoomFactor);
}
